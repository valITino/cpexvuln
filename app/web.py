...
def create_app(args):
    ...
    def run_watchlist(wid):
        ...
        current = next((x for x in wl["lists"] if x["id"] == wid), None)
        if not current:
            flash("Watchlist not found."); return redirect(url_for("index"))
        win = (request.args.get("win") or "24h").lower()
        force_since = now_utc() - (timedelta(hours=DAILY_LOOKBACK_HOURS) if win=="24h" else timedelta(days=LONG_BACKFILL_DAYS))
        ...
        return render_template_string(TEMPLATE, watchlists=wl["lists"], current=current, results=results,
    ...
    def submit():
        ...
        if not cpes:
            flash("Could not parse any CPEs."); return redirect(url_for("index"))
        ...
        if action == "save_only":
            flash("Saved."); return redirect(url_for("open_watchlist", wid=wid))
        return redirect(url_for("run_watchlist", wid=wid, win=("24h" if action=="run_daily" else "90d")))
    ...
    def export_csv(wid):
        ...
        buf = io.StringIO()
        w = csv.writer(buf)
        w.writerow(["CVE","Severity","Score","Published","LastModified","MatchedCPE","KEV","CWEs","Description"])
        for r in results:
            sev = r.get("severity","")
            score = r.get("score","")
            w.writerow([r.get("cve",""), sev, score, r.get("published",""), r.get("lastModified",""),
                        r.get("matched_cpe_query",""), "yes" if r.get("kev") else "", ";".join(r.get("cwes",[])),
